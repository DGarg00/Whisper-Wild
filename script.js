...
class Engine {
  ...
  /* build one live instance for a sound name. returns {out, stop, analyser} */
  build(name){
    this.ensure();
    const ctx = this.ctx;
...
const state = { active: new Map() }; // name -> {out, stop, analyser, els:[]}

function toggleSound(name, cardEls){
...
function stopAll(){
  state.active.forEach(inst=>{ inst.stop(); inst.els.forEach(el=>el.classList.remove('is-active')); });
  state.active.clear();
  renderNowPlaying();
}
  ...
  COMBOS.items.forEach(combo=>{
    ...
    btn.addEventListener('click', ()=>{
      ...
      combo.layers.forEach(layerName=>{
        const already = state.active.has(layerName+'::'+combo.title);
        if (comboOn && !already){
          const inst = engine.build(layerName);
          inst.els = [btn];
          state.active.set(layerName+'::'+combo.title, inst);
        } else if (!comboOn && already){
          state.active.get(layerName+'::'+combo.title).stop();
          state.active.delete(layerName+'::'+combo.title);
        }
      });
      btn.classList.toggle('is-active', comboOn);
      renderNowPlaying();
    });
    grid.appendChild(btn);
  });
